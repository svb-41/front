...
export const ai: svb.AI<Data> = ({ stats, radar, memory, ship, comm }) => {
  if (!memory.initialPos) memory.initialPos = stats.position
  ...
  if (near) {
    const source = stats.position
    const target = svb.geometry.nextPosition(200)(near.enemy.position)
  ...
  if (svb.geometry.dist2(stats.position, memory.initialPos) > 400) {
    const source = stats.position
    const target = memory.initialPos
    return svb.geometry.turnToTarget({ source, target, ship })
  }
  return ship.idle()
}
